...
const webApp = tcb.init({});
const webAuth = webApp.auth();
...
// Web Phase 1 只开放当前 PC 后台实际使用的接口。
// 即使有人在浏览器控制台手工调用，也不能触发小程序专属/尚未开放的管理、结算等操作。
const WEB_ALLOWED_ACTIONS = new Set([
  'bindWebEmployee', 'getSessionUser', 'getCities', 'getSources', 'getOrders', 'getOrderDetail',
  'createOrder', 'getWorkers', 'updateOrder', 'editTime', 'cancelOrder', 'urgent', 'feedback'
]);

const WEB_PUBLIC_ERROR_CODES = new Set([
  'WEB_AUTH_REQUIRED', 'INVALID_PHONE', 'WEB_BINDING_CONFLICT', 'WEB_ROLE_DENIED',
  'WEB_ALREADY_BOUND', 'WEB_PHONE_NOT_VERIFIED', 'EMPLOYEE_NOT_FOUND',
  'EMPLOYEE_PHONE_CONFLICT', 'EMPLOYEE_WEB_BOUND', 'CREATE_OPERATION_ID_CONFLICT',
  'FEEDBACK_OPERATION_ID_CONFLICT', 'ORDER_VERSION_CONFLICT', 'INVALID_ORDER_VERSION',
  'ORDER_NOT_FOUND', 'ORDER_ACCESS_DENIED', 'ORDER_ARCHIVED', 'ORDER_STATE_CHANGED'
]);
...
function normalizeWebLoginPhone(value) {
...
function requestIdentity() {
  // CloudBase Web SDK 调用云函数后，当前登录用户身份由 Node SDK 注入。
  // 官方方式是 auth.getUserInfo()，不要从 context.auth 猜测 UID。
  let info = {};
  try {
    info = webAuth.getUserInfo() || {};
  } catch (error) {
    console.warn('读取 Web 调用者身份失败：', error && error.message);
  }
  return {
    openid: String(info.openId || info.openid || '').trim(),
    uid: String(info.uid || '').trim()
  };
}

async function currentMiniProgramUser(openid) {
...
async function currentWebUser(uid) {
  if (!uid) return null;

  // Web 身份只按已经绑定的 CloudBase UID 定位。
  // 首次绑定必须显式走 bindWebEmployee，并由服务端验证 CloudBase 账号确实拥有该手机号。
  const bound = await db.collection('users').where({ webUid: uid }).limit(2).get();
  if ((bound.data || []).length === 1) {
    const user = bound.data[0];
    return ['admin', 'leader', 'service'].includes(user.role) ? user : null;
  }
  if ((bound.data || []).length > 1) {
    console.error('检测到同一 Web UID 绑定多个员工：', uid);
  }
  return null;
}

async function verifyWebUidOwnsPhone(uid, phone) {
  const normalizedPhone = normalizeWebLoginPhone(phone);
  if (!uid || !normalizedPhone) return false;

  // 使用 CloudBase 官方 queryUserInfo(PHONE) 反查手机号登录标识；
  // 只有该手机号对应的 uid 与当前函数调用者 uid 完全一致，才允许绑定员工。
  const platformIds = [`+86 ${normalizedPhone}`, `+86${normalizedPhone}`, normalizedPhone];
  for (const platformId of platformIds) {
    try {
      const result = await webAuth.queryUserInfo({ platform: 'PHONE', platformId });
      const info = result && result.userInfo ? result.userInfo : {};
      const matchedUid = String(info.uid || '').trim();
      if (matchedUid && matchedUid === uid) return true;
    } catch (error) {
      // 某一种手机号格式查不到时继续尝试其他官方常见格式。
    }
  }
  return false;
}

async function bindWebEmployee(uid, rawPhone) {
  const phone = normalizeWebLoginPhone(rawPhone);
  if (!uid) throw makeOrderError('WEB_AUTH_REQUIRED', '网页登录已失效，请重新登录');
  if (!phone) throw makeOrderError('INVALID_PHONE', '请输入正确的员工手机号');

  const alreadyBound = await db.collection('users').where({ webUid: uid }).limit(2).get();
  const boundUsers = alreadyBound.data || [];
  if (boundUsers.length > 1) {
    throw makeOrderError('WEB_BINDING_CONFLICT', '该网页账号绑定状态异常，请联系管理员处理');
  }
  if (boundUsers.length === 1) {
    const boundUser = boundUsers[0];
    if (!['admin', 'leader', 'service'].includes(boundUser.role)) {
      throw makeOrderError('WEB_ROLE_DENIED', '该员工角色不能使用电脑端后台');
    }
    if (String(boundUser.phone || '') !== phone) {
      throw makeOrderError('WEB_ALREADY_BOUND', '该网页账号已绑定其他员工，不能切换员工身份');
    }
    return boundUser;
  }

  // 不能只相信浏览器传来的手机号；必须由 CloudBase Auth 证明当前 uid 确实拥有该手机号。
  const ownsPhone = await verifyWebUidOwnsPhone(uid, phone);
  if (!ownsPhone) {
    throw makeOrderError(
      'WEB_PHONE_NOT_VERIFIED',
      '当前 CloudBase 登录账号未绑定此员工手机号，请确认身份认证中的手机号与员工手机号一致'
    );
  }

  const employeeResult = await db.collection('users').where({ phone }).limit(2).get();
  const employees = employeeResult.data || [];
  if (employees.length === 0) throw makeOrderError('EMPLOYEE_NOT_FOUND', '未找到对应员工，请先在员工管理中创建该员工');
  if (employees.length > 1) throw makeOrderError('EMPLOYEE_PHONE_CONFLICT', '该手机号对应多个员工，请联系管理员处理');

  const employee = employees[0];
  if (!['admin', 'leader', 'service'].includes(employee.role)) {
    throw makeOrderError('WEB_ROLE_DENIED', '该员工角色不能使用电脑端后台');
  }

  // 事务内再次读取员工记录，避免两个 Web 账号同时抢绑同一员工。
  const transaction = await db.startTransaction();
  try {
    const latest = await transaction.collection('users').doc(employee._id).get();
    const current = latest.data;
    if (!current) throw makeOrderError('EMPLOYEE_NOT_FOUND', '员工不存在');
    if (current.webUid && String(current.webUid) !== uid) {
      throw makeOrderError('EMPLOYEE_WEB_BOUND', '该员工已经绑定其他网页账号');
    }
    if (!current.webUid) {
      await transaction.collection('users').doc(employee._id).update({ data: {
        webUid: uid,
        webBoundAt: new Date().toISOString()
      } });
    }
    await transaction.commit();
    return { ...current, webUid: uid };
  } catch (error) {
...
async function currentUser(identity = {}) {
  if (identity.openid) return await currentMiniProgramUser(identity.openid);
  if (identity.uid) return await currentWebUser(identity.uid);
  return null;
}
...
exports.main = async (event = {}, context = {}) => {
  ...
  try {
    // Web 云函数只接受 CloudBase Web Auth UID；小程序 OPENID 和定时 Trigger 均不作为身份入口。
    if (!identity.uid || identity.openid || event.TriggerName) {
      return { success: false, code: 'WEB_AUTH_REQUIRED', msg: '网页登录已失效，请重新登录' };
    }
    if (!WEB_ALLOWED_ACTIONS.has(action)) {
      return { success: false, code: 'WEB_ACTION_DENIED', msg: '网页端未开放此操作' };
    }

    // Web 首次绑定：在 users.webUid 尚不存在时，由 CloudBase UID + 已验证手机号完成绑定。
    if (action === 'bindWebEmployee') {
      const boundUser = await bindWebEmployee(identity.uid, data && data.phone);
      return { success: true, user: clientUser(boundUser) };
    }

    const user = await currentWebUser(identity.uid);
    if (!user) return { success: false, code: 'UNAUTHORIZED', msg: '网页账号尚未关联员工或登录已失效' };

    // Web 基础数据要求有效登录；其余未列入 WEB_ALLOWED_ACTIONS 的操作在入口即拒绝。
    if (action === 'getCities') return { success: true, cities: await getReferenceCities() };
    if (action === 'getSources') return { success: true, sources: await getReferenceSources() };
    ...
    console.error('manageOrderWeb 处理异常：', err);
    if (err && WEB_PUBLIC_ERROR_CODES.has(err.code)) {
      return { success: false, code: err.code, msg: err.message || '操作失败' };
    }
    // 不把内部异常细节返回给浏览器，避免泄露数据库/SDK/运行环境信息。
    return { success: false, code: 'INTERNAL_ERROR', msg: '云端处理失败，请稍后重试' };
  }
};
